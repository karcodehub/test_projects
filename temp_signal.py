...
# Generate 
    ...
    #    plt.show()
    symb_last = y2_axis[int(-(100)):]
    #non_convg=y2_axis[-500:]
    non_convg=y2_axis[::-1]
    ...
    if (len(non_convg_val) != 0):
        ...
        print(int(non_convg_val[0]))
        y3_axis[i]= int(non_convg_val[0])
    else:
        y3_axis[i]=1000
...
plt.plot(x3_axis, y3_axis , marker="+", label = 'samps out of bound', color='b')
plt.show()
